...
single_fre=raw_input('1.ISW 2.dust 3.subtracted\n')
...
def written_as_a_function_to_save_memory(z_bins,catalogname,single_fre):
    ...
    if single_fre=='1':
        ...
        planck2048=healpy.pixelfunc.ud_grade(planckImap,2048,order_in='NEST', order_out='NEST')
        planck2048mask=healpy.pixelfunc.ud_grade(planckImask,2048,order_in='NEST', order_out='NEST')
        ...
        planckImap=healpy.pixelfunc.get_interp_val(planckImap ,planck_ra ,planck_dec ,nest=True ,lonlat=True)

        coord=SkyCoord(planck_ra,planck_dec,frame='galactic',unit='deg').icrs
        planck_ra,planck_dec=coord.ra.deg,coord.dec.deg

        planckset=np.array([planckImap,planck_ra,planck_dec])
        planckset=planckset[:,(planckset[0]>=Imin)&(planckset[0]<=Imax)]
        planckImap,planck_ra,planck_dec=planckset[0],planckset[1],planckset[2]
    ...
    if single_fre=='3':
        planckdata=fits.open('http://pla.esac.esa.int/pla/aio/product-action?MAP.MAP_ID=HFI_CompMap_Foregrounds-commander-143_R3.00.fits')
    ...
    if single_fre=='1':
        if catalogname=='1':
            np.save('datalog/ISW_z_{}.npy'.format(str(z_bins)),np.array([xi,r,sig]))
            print('datalog saved!')

    if single_fre=='2':
        if catalogname=='1':
            np.save('datalog/CIB_z_{}.npy'.format(str(z_bins)),np.array([xi,r,sig]))
            print('datalog saved!')

    if single_fre=='3':
        if catalogname=='1':
            np.save('datalog/Subtracted_z_{}.npy'.format(str(z_bins)),np.array([xi,r,sig]))
            print('datalog saved!')
...
for z_bins in range(4):
    written_as_a_function_to_save_memory(z_bins,catalogname,single_fre)
'''

written_as_a_function_to_save_memory(1,catalogname,single_fre)
