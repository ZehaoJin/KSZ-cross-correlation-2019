...
if catalogname=='2':
    ...
    rand_ra = np.random.uniform(ra_min, ra_max, MCXCdata['RA'].size)
    rand_sindec = np.random.uniform(np.sin(dec_min), np.sin(dec_max), MCXCdata['RA'].size)
    rand_dec = np.arcsin(rand_sindec)
...
#'''
nk = treecorr.NKCorrelation(min_sep=0.01, max_sep=10, nbins=30, sep_units='deg')
rk = treecorr.NKCorrelation(min_sep=0.01, max_sep=10, nbins=30, sep_units='deg')
...
#plt.xlim([0,10])
plt.title('Planck x SuperCosmos')
plt.show()
